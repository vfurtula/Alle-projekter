...
class Run_CM110_Thread(QThread):
	...
	def run(self):
		...
		with open(self.my_file, 'a') as thefile:
			thefile.write(''.join([ "Wavelength[nm], X voltage[", self.unit_str, "],  Ard voltage[", self.unit_str, "], Ard raw data[0..1023 steps], ", "Timetrace [sec]\n"]))

		#make a heading for the datafile with all the Arduino outputs
		with open(self.elapsedtime_plot, 'a') as thefile:
			thefile.write(''.join([ "Wavelength[nm], Ard voltage[", self.unit_str, "], Ard data[0..1023 steps], ", "Timetrace [sec]\n"]))
			...
			for new_position in wv_scanlist:
				...
				for ii in range(dwelltimerat):
					...
					if ii==dwelltimerat-1:
						...
						X_val=self.sr850.return_X()/self.norm_voltage(self.unit_str)
						with open(self.my_file, 'a') as thefile:
							thefile.write("%s " % new_position)
...
class Run_CM110(QtGui.QWidget):
	...
	def set_run(self):
		...
		if not self.unit_str in ['pV','nV','uV','mV','V']:
			QtGui.QMessageBox.warning(self, 'Message',"The voltage unit should be pV, nV, uV, mV or V!")
			raise ValueError("The voltage unit should be pV, nV, uV, mV or V!")
		...
		self.get_thread=Run_CM110_Thread(start_sc,stop_sc,step_sc,dwelltime,dwell_ard,self.avg_pts,
																	 expand,offset,sens,self.unit_str,filename,folder,self.timestr,self.analogref,self.CM110,self.Ard,self.SR850)
	...
	def set_connect(self):
		...
		try:
			...
			#self.SR850.set_timeout(0.1)
			self.SR850.set_to_rs232()
		except Exception, e:
			QtGui.QMessageBox.warning(self, 'Message',
														 "No response from SR850 lock-in serial port! Check the port name and check the power line.")
			self.conMode.setEnabled(True)
			self.disconMode.setEnabled(False)
			self.allEditFields(False)
