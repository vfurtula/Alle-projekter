...
class zaber_Thread(QThread):
	...
	def __init__(self, *argv):
		...
		self.Za=self.argv[1]
		self.Ard=self.argv[2]
		self.calib_file=self.argv[3]
		self.avg_pts=self.argv[4]
	...
	def run(self):
		...
		if self.sender==u'\u25b2':
			...
			alignEdit_nm=self.argv[5]
			min_pos=self.get_pos(alignEdit_nm)
			self.Za.set_Current_Position(1,1,min_pos)
			bit_step=self.Ard.get_val(self.avg_pts)
			self.emit(SIGNAL('more_tals(QString,QString,QString)'), str(min_pos), str(alignEdit_nm), str(bit_step))
			with open('zaber_position.py', 'w') as thefile:
			  thefile.write( ''.join(['Position=',str(min_pos),'\n']) )
						
		elif self.sender=='Start scan':
			# open calib file and plot
			self.start_ = self.argv[5]
			self.stop_ = self.argv[6]
			self.step_ = self.argv[7]
			self.dwell_time = self.argv[8]
			self.dwell_time_ard = self.argv[9]/1000.0 #convert to milliseconds
			self.save_to_file = self.argv[10]
			self.analogref = self.argv[11]
			...
			for start,stop,step,dwell in zip(self.start_,self.stop_,self.step_,self.dwell_time):
				...
				for ff,pp in zip(wv_scanlist,positions):
					...
					for ii in range(divs):
						...
						if ii==divs-1: # save only last element from the loop above
...
class run_zaber_gui(QtGui.QWidget):
	...
	def set_zero(self):
		...
		if move_num<self.min_y_calib or move_num>self.max_y_calib:
			...
			sender = self.sender()
			get_zaber_Thread=zaber_Thread(sender.text(),self.Za,self.Ard,self.calib_file_str,self.avg_pts,move_num)
