...
class run_zaber_gui(QtGui.QWidget):
	...
	def set_connect(self):
		...
		micstep=self.Za.set_Microstep_Resolution(2*64)
		max_pos=self.Za.set_Maximum_Position(81000)
	...
	def move_stop(self):
		...
		sender = self.sender()
		get_zaber_Thread2=zaber_Thread(sender.text(),self.Za,self.Ard,str(self.calibfileEdit.text()),self.avg_pts)
		self.connect(get_zaber_Thread2,SIGNAL("more_tals(QString,QString,QString)"),self.more_tals)
		self.connect(get_zaber_Thread2,SIGNAL('finished()'), self.set_finished)
		
		get_zaber_Thread2.start()

	def set_zero(self):
		
		sender = self.sender()
		get_zaber_Thread3=zaber_Thread(sender.text(),self.Za,self.Ard,str(self.calibfileEdit.text()),self.avg_pts,int(self.calibEdit.text()),0)
		self.connect(get_zaber_Thread3,SIGNAL("more_tals(QString,QString,QString)"),self.more_tals)
		self.connect(get_zaber_Thread3, SIGNAL('finished()'), self.set_finished)

		get_zaber_Thread3.start()    
