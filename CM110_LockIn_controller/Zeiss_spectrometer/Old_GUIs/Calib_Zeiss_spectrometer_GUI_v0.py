...
class zaber_Thread(QThread):
	...
	def run(self):
		...
		if self.sender==u'\u25b2':
			...
			self.emit(SIGNAL('more_tals(QString,QString)'), str(min_pos), str(round(self.move_num,1)))
			with open('zaber_position.py', 'w') as thefile:
			  thefile.write( ''.join(['Position=',str(min_pos),'\n']) )
...
class run_zaber_gui(QtGui.QWidget):
	...
	def initUI(self):
		...
		try:
			...
			QtGui.QMessageBox.warning(self, 'Message', 
														"Something is wrong with the wavelength file! Do you have a wavelength file with 1 column, no headers, and all inputs are digits?")
	...
	def set_connect(self):
		...
		try:
			...
			QtGui.QMessageBox.warning(self, 'Message',
														 "No response from the Zaber serial port! Check the Zaber port name and check the power lines.")
		...
		# constants
		self.Za.set_Current_Position(self.all_pos[-1])
	...
	def move_stop(self):
		...
		sender = self.sender()
		get_zaber_Thread=zaber_Thread(sender.text(),self.Za,self.calib_file_str)
