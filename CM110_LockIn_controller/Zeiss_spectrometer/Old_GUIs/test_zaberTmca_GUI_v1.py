...
class zaber_Thread(QThread):
	...
	def __init__(self, *args):
		...
		if len(args)==3:
		  self.sender=args[0]
		  self.Za=args[1]
		  self.calib_file=args[2]
		elif len(args)==4:
		  self.sender=args[0]
		  self.Za=args[1]
		  self.calib_file=args[2]
		  self.move_num=args[3]
		elif len(args)==7:
			self.sender=args[0]
			self.Za=args[1]
			self.calib_file=args[2]
	...
	def run(self):
			...
			if not self.calib_file:
				...
				if not x:
					...
					for ff,pp in zip(wv_scanlist,positions):
						...
						self.emit(SIGNAL('time_data(QString,QString,QString)'), str(time_elap),str(ff),str(min_pos))
		...
		elif self.sender=='quit':
		  min_pos=self.Za.return_Current_Position()
...
class run_zaber_gui(QtGui.QWidget):
	...
	def initUI(self):
		...
		self.pw1.setLabel('bottom', 'no. of moves')
		...
		self.pw3.setLabel('left', 'wavelength', units='nm')
		...
		self.curve4=self.p1.plot(pen='w')
		...
		self.curve3=self.p3.plot(pen='m')
	...
	def set_disconnect(self):
		...
		self.get_zaber_Thread=zaber_Thread('quit',self.Za,str(self.calibfileEdit.text()))
		self.get_zaber_Thread.start()
		while self.get_zaber_Thread.isRunning():
			pass
		self.Za.close()
		...
		if reply == QtGui.QMessageBox.Yes:
			if hasattr(self, 'Za'):
				if self.connectButton.isEnabled()==False:
					#self.Za = test_zaberTmca.Zaber_Tmca()
					get_zaber_Thread=zaber_Thread('quit',self.Za,str(self.calibfileEdit.text()))
					get_zaber_Thread.start()
					while get_zaber_Thread.isRunning():
						pass
					self.Za.close()
					event.accept()
				else:
	...
	def set_scan(self):
		...
		sender = self.sender()
		self.get_zaber_Thread=zaber_Thread(sender.text(),self.Za,str(self.calibfileEdit.text()),start_sc,stop_sc,step_sc,dwell_zaber)
	...
	def move_stop(self):
		...
		sender = self.sender()
		get_zaber_Thread=zaber_Thread(sender.text(),self.Za,str(self.calibfileEdit.text()))
	...
	def move_to_nm(self):
		...
		sender = self.sender()
		self.get_zaber_Thread=zaber_Thread(sender.text(),self.Za,str(self.calibfileEdit.text()),move_num)
		self.connect(self.get_zaber_Thread,SIGNAL("more_tals(QString,QString)"),self.more_tals)
		self.connect(self.get_zaber_Thread, SIGNAL('finished()'), self.set_finished)
	...
	def move_jog(self):
		...
		# update the lcd motorstep position
		sender = self.sender()
		self.get_zaber_Thread=zaber_Thread(sender.text(),self.Za,str(self.calibfileEdit.text()))
		self.connect(self.get_zaber_Thread,SIGNAL("more_tals(QString,QString)"),self.more_tals)
		self.connect(self.get_zaber_Thread, SIGNAL('finished()'), self.set_finished)
	...
	def time_data(self,time,iii,pos):
		
		self.all_wv.extend([ int(iii) ])
		self.all_pos.extend([ int(pos) ])
		self.all_time.extend([ float(time) ])
		self.curve4.setData(self.all_wv)
		self.curve3.setData(self.all_time,self.all_wv)
