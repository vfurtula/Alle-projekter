...
class zaber_Thread(QThread):
	...
	def run(self):
		...
		if self.sender==u'\u25b2':
			...
			self.replace_line("config_zeiss.py",6,''.join(["Last_position=[",str(min_pos),",",str(self.move_num),"]\n"]))
			imp.reload(config_zeiss)
...
class run_zaber_gui(QtGui.QWidget):
	...
	def __init__(self):
		...
		self.wavelength_str = config_zeiss.wavelengthfile
		self.calib_file_str = config_zeiss.calibfile
		self.filename_str = config_zeiss.calib_filename
		self.folder_str = config_zeiss.calib_foldername
	...
	def initUI(self):
		...
		_, tail_ = os.path.split(self.calib_file_str)
		self.lbl2 = QtGui.QLabel("", self)
		self.lbl2.setText( ''.join(["<font color=\"black\">Calib file: </font> <font color=\"green\">",tail_,"<"]) )
		
		_, tail_ = os.path.split(self.wavelength_str)
		self.lbl3 = QtGui.QLabel("", self)
		self.lbl3.setText( ''.join(["<font color=\"black\">Wavelength file: </font> <font color=\"green\">",tail_,"<"]) )
		...
		g8_0.addWidget(self.lbl2)
		g8_0.addWidget(self.lbl3)
		g8_0.addWidget(lbl4)
	...
	def loadWaveDialog(self):
		...
		fname = QtGui.QFileDialog.getOpenFileName(self, 'Open file','Wavelength_files')
		if fname:
	...
	def set_save(self):
		...
		self.replace_line("config_zeiss.py",8, ''.join(["wavelengthfile=\"",self.wavelength_str,"\"\n"]) )
		self.replace_line("config_zeiss.py",11, ''.join(["calib_filename=\"",str(self.filenameEdit.text()),"\"\n"]) )
		self.replace_line("config_zeiss.py",12, ''.join(["calib_foldername=\"",str(self.folderEdit.text()),"\"\n"]) )
