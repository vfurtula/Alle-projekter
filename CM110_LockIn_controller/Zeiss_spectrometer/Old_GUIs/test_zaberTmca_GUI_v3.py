	...
	def __init__(self, *args):
		...
		if len(args)==5:
		  ...
		  self.avg_pts = args[5]
			...
			self.avg_pts = args[9]
			self.save_to_file = args[10]
			self.analogref = args[11]
		else:
		  pass
					...
					for start,stop,step,dwell in zip(self.start_,self.stop_,self.step_,self.dwell_time):
						...
						for ff,pp in zip(wv_scanlist,positions):
							...
							for ii in range(divs):
								...
								bit_step=self.Ard.get_val(self.avg_pts)
								self.emit(SIGNAL('volt_data(QString,QString,QString)'), str(time_elap),str(ff),str(self.analogref*(bit_step/1023)))
								#time.sleep(self.dwell_time_ard)
								while (time.time()-time_ard_start)<self.dwell_time_ard:
									pass
								
							self.emit(SIGNAL('time_data(QString,QString,QString,QString)'), str(time_elap),str(ff),str(min_pos),str(self.analogref*(bit_step/1023)))
							with open(self.save_to_file, 'a') as thefile:
								thefile.write("%s" % ff)
								thefile.write("\t%s" % int(self.analogref*(bit_step/1023)))
								thefile.write("\t%s" % bit_step)
								thefile.write("\t%s\n" % time_elap)
								
		elif self.sender=='Zaber->100':
			min_pos=100
...
class run_zaber_gui(QtGui.QWidget):
	...
	def initUI(self):
		...
		self.setzeroButton = QtGui.QPushButton('Zaber->100',self)
		self.setzeroButton.setFixedWidth(90)
	...
	def set_scan(self):
		...
		sender = self.sender()
		self.get_zaber_Thread=zaber_Thread(sender.text(),self.Za,self.Ard,str(self.calibfileEdit.text()),start_sc,stop_sc,step_sc,dwell_zaber,dwell_ard,self.avg_pts,self.save_to_file,self.analogref)
	...
	def move_to_nm(self):
		...
		sender = self.sender()
		self.get_zaber_Thread=zaber_Thread(sender.text(),self.Za,self.Ard,str(self.calibfileEdit.text()),move_num,self.avg_pts)
		self.connect(self.get_zaber_Thread,SIGNAL("more_tals(QString,QString,QString)"),self.more_tals)
		self.connect(self.get_zaber_Thread, SIGNAL('finished()'), self.set_finished)
