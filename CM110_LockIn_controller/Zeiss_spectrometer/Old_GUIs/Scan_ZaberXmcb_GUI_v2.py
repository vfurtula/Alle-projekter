...
class zaber_Thread(QThread):
	...
	def update(self):
		...
		self.emit(SIGNAL('more_tals(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), min_pos, pos_nm, sr510_volt)
	...
	def run(self):
		...
		if self.sender==u'\u25b2':
			...
			sr510_volt=self.Sr510.return_voltage()
			self.emit(SIGNAL('more_tals(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), min_pos, alignEdit_nm, sr510_volt)
			with open('zaber_position.py', 'w') as thefile:
			  thefile.write( ''.join(['Position=',str(min_pos),'\n']) )
			...
			for start,stop,step,dwell in zip(self.start_,self.stop_,self.step_,self.dwell_time):
				...
				for ff,pp in zip(wv_scanlist,positions):
					...
					sr510_volt=self.Sr510.return_voltage()
					self.emit(SIGNAL('more_tals(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), min_pos, pos_nm, sr510_volt)	
					self.emit(SIGNAL('curve3_data(PyQt_PyObject,PyQt_PyObject)'), min_pos, ff)
					with open('zaber_position.py', 'w') as thefile:
					  thefile.write( ''.join(['Position=',str(min_pos),'\n']) )
					...
					while (time.time()-time_s)<dwell:
						...
						self.emit(SIGNAL('volt_data(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), time_elap,ff,sr510_volt)
					...
					with open(self.save_to_file, 'a') as thefile:
						...
						thefile.write("\t%s" %sr510_volt)
						thefile.write("\t%s\n" %time_elap)
					self.emit(SIGNAL('time_data(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), time_elap,ff,sr510_volt)
