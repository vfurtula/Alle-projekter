...
class Fast_logdata:
  def __init__(self, start_CM, stop_CM, step_CM, wait_CM, intg_pts, string_filename):
		# constants
		self.outputs=1
		self.start=start_CM
		self.stop=stop_CM
		...
		#Write the number of integration points to the Arduino serial
		time.sleep(1.5)
		self.ser_ARD.write(chr(43))
		time.sleep(0.1)
		if self.pts==0 or self.pts>255:
			raise ValueError("The number of integration points is minimum 1 and maximum 255")
		else:
			self.ser_ARD.write(chr(self.pts))
		time.sleep(0.1)
		
		#UNITS
		self.ser_CM.write(chr(50))
		time.sleep(0.1)
		self.ser_CM.write(chr(01)) #set units to nm
		...
		if self.start>=0 and self.start<=9000:
			...
			multiplicator=self.start/256
			runtohere=self.start-256*multiplicator
			#send the parameters to the device
			self.ser_CM.write(chr(multiplicator)) #highbyte, 0 for numbers below 256
			time.sleep(0.1)
			self.ser_CM.write(chr(runtohere)) #go to the start position
		...
		#STEPSIZE
		self.ser_CM.write(chr(55)) #call STEP
		time.sleep(0.1)
		self.ser_CM.write(chr(self.step)) # stepsize in nm
		time.sleep(0.1)
  ...
  def logdata(self):
		...
		with open(self.my_file, 'a') as thefile:
			thefile.write( ''.join(['Start=',str(self.start),'nm, ', 'Stop=',str(self.stop),'nm, ','Step=',str(self.step),'nm, ','Wait-time=',str(self.wait),'s, ','Intg.pts.=',str(self.pts), '\n']) )
			thefile.write('Wavelength[nm], Voltage[10 bits=1024 steps]\n')
...
if __name__ == "__main__":
	...
	if len(sys.argv)==7:
		...
		print 'Argument 5: Number of integration points for Arduino, range: 1-255'
	...
	Ld = Fast_logdata(start_CM, stop_CM, step_CM, wait_CM, intg_pts, string_filename)
	timelist=Ld.logdata()
	...
	analogRef=5.0 # if analogRef (Arduino 2560) is not enabled than analogRef=5 Volts
	ax.plot(data1, timelist, 'k-')
	ax2 = ax.twinx()
	ax2.plot(data1, analogRef*np.array(data2)*1000/2**10, 'r-')
