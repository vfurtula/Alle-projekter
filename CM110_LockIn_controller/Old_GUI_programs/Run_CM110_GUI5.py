...
class runCM110(QThread):
	...
	def __init__(self):
		...
		reload(config_CM110)
		# constants
		self.start_ = config_CM110.Start
		self.stop = config_CM110.Stop
		self.step = config_CM110.Step
		self.dwell_time = config_CM110.Wait_time
		self.dwell_time_ard = config_CM110.Wait_time_ard
	...
	def run(self):
		...
		if isinstance(self.start_, float):
		...
		if self.dwell_time<1 or self.dwell_time>100:
		...
		if self.start_>=0 and self.start_<=9000:
			...
			multiplicator=self.start_/256 #highbyte, 0 for numbers below 256
			runtohere=self.start_-256*multiplicator 
			#send the parameters to the device
			self.ser_CM.write([chr(tal) for tal in [16,multiplicator,runtohere] ])
			
			print "...positioning scanner to the wavelength", self.start_, "nm"
			time.sleep(5)
		else:
		...
		with open(self.my_file, 'a') as thefile:
			thefile.write( ''.join(["Start=",str(self.start_),"nm, ", "Stop=",str(self.stop),"nm, ","Step=",str(self.step),"nm, ","Dwell time=",str(self.dwell_time),"s, ","Avg.pts.=",str(self.avg_pts), "\n"]) )
		...
		dwelltimerat=(self.dwell_time*1000)/self.dwell_time_ard
		for new_position in range(self.start_,self.stop+self.step,self.step):
...
class CM110(QtGui.QWidget):
	...
	def initUI(self): 
		...
		self.startEdit = QtGui.QLineEdit(str(self.start_),self)
