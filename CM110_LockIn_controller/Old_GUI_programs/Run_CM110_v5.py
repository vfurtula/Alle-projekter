		...
		try:
			...
			while(new_position<self.stop):
				...
				while (timetot<=self.wait):
					...
					# Save stepped wavelengths in a textfile				
					with open(self.my_file, 'a') as thefile:
						...
						with open(self.my_file, 'a') as thefile:
							for tal in range(len(self.data)):
								if tal == len(self.data)-1:
									thefile.write("\t%s" % (analogRef*self.data[tal]/1023))
									thefile.write("\t%s" % self.data[tal])
									thefile.write("\t%s\n" % (timebeg-time_start))
								else:
									thefile.write("\t%s" % (analogRef*self.data[tal]/1023))
									thefile.write("\t%s" % self.data[tal])
									thefile.write("\t%s" % (timebeg-time_start))
...
if __name__ == "__main__":
	...
	ax.plot(timelist, data1, 'k-')
	ax2 = ax.twinx()
	ax2.plot(timelist, analogRef*np.array(data3)/1023, 'r-')
