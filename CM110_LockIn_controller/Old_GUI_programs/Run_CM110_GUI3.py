...
class runCM110(QThread):
	...
	def run(self):
		...
		while(new_position<self.stop_str):
			...
			self.emit(SIGNAL('make_update1(QString,QString,QString)'), 
						 str(new_position),str(all_data[-1]),str(timelist[-1]))
			with open(self.my_file, 'a') as thefile:
				thefile.write("%s " % new_position)
...
class CM110(QtGui.QWidget):
	...
	def initUI(self): 
		...
		self.status_str = QtGui.QLabel("ALWAYS SAVE AFTER EDITING SETTINGS!", self)
	...
	def set_run(self):
		...
		self.all_volts=[]
		self.all_times=[]
		self.curve1.setData(self.all_wl, self.all_volts)
		...
		self.all_volts_tr=[]
		self.all_time_tr=[]
		self.curve2.setData(self.all_time_tr, self.all_volts_tr)
		self.curve3.setData(self.all_times, self.all_volts)
		self.curve4.setData(self.all_time_tr, self.all_wl_tr)
	...
	def make_update1(self,all_positions,endpoint_data,endpoints_times):
