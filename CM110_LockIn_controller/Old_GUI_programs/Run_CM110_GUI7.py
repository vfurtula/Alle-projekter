...
class Run_CM110(QtGui.QWidget):
	...
	def set_run(self):
		...
		self.get_thread=Run_CM110_Thread(sender.text(),start_sc,stop_sc,step_sc,dwelltime,dwell_ard,
																	 self.avg_pts,expand,offset,sens,self.unit_str,filename,folder,self.timestr,self.analogref,self.CM110,self.Ard)
	...
	def set_cancel(self):
		...
		sender = self.sender()
		get_thread=Run_CM110_Thread(sender.text(),self.CM110)
		self.connect(get_thread, SIGNAL('finished()'), self.set_finished)
		get_thread.start()
