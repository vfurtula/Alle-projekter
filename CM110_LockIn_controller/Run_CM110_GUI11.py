...
import SR850, CM110, ArdMega2560
...
class Run_CM110_Thread(QThread):
	...
	def __init__(self,*argv):
		...
		self.offset = argv[7]
		self.sens = argv[8]
		self.filename_str = argv[9]
		self.folder_str = argv[10]
		self.timestr = argv[11]
		self.analogRef = argv[12]		
		self.cm110 = argv[13]
		self.Ard = argv[14]
		self.sr850 = argv[15]
	...
	def norm_voltage(self,my_str):
		
		all_volts=['pV','nV','uV','mV','V']
		all_vals=[1e-12,1e-9,1e-6,1e-3,1]
		if my_str in all_volts:
			ind=all_volts.index(my_str)
			return all_vals[ind]
		else:
			raise ValueError("The voltage unit should be pV, nV, uV, mV or V!")
	...
	def run(self):
		...
		with open(self.my_file, 'a') as thefile:
			thefile.write(''.join([ "Wavelength[m], X voltage[V],  Ard voltage[V], Ard raw data[0..1023 steps], ", "Timetrace [sec]\n"]))

		#make a heading for the datafile with all the Arduino outputs
		with open(self.elapsedtime_plot, 'a') as thefile:
			thefile.write(''.join([ "Wavelength[m], Ard voltage[V], Ard data[0..1023 steps], ", "Timetrace [sec]\n"]))
				...
				for ii in range(dwelltimerat):
					...
					if ii==dwelltimerat-1:
						...
						with open(self.my_file, 'a') as thefile:
							...
							thefile.write("\t%s" % bit_step)
							thefile.write("\t%s\n" % time_elap)
						self.emit(SIGNAL('make_update1(QString,QString,QString,QString,QString)'),str(new_position),str(X_val),str(Vlockin),str(time_elap),str(bit_step))	
						# Do STEP
...
class Run_CM110(QtGui.QWidget):
	...
	def __init__(self):
		...
		self.sr850port_str=config_Run_CM110.sr850port
	...
	def initUI(self):
		...
		self.serialSR850 = serialMenu.addAction("SR850")
		self.serialSR850.triggered.connect(self.SR850Dialog)
    ...
    # PLOT 1 settings
		...
		self.curve6=self.p0.plot(pen='m')
		...
		self.curve7=self.p1.plot(pen='m')
		# create plot and add it to the figure
		self.p2 = pg.ViewBox()
	...
	def SR850Dialog(self):

		text, ok = QtGui.QInputDialog.getText(self, 'Serial Port Dialog',
																				'Enter SR850 serial:', text=self.sr850port_str)
		if ok:
			self.sr850port_str = str(text)
	...
	def set_run(self):
		...
		self.disconMode.setEnabled(False)
		self.serialCM110.setEnabled(False)
		self.serialArd.setEnabled(False)
		self.serialSR850.setEnabled(False)
		
		self.get_thread=Run_CM110_Thread(start_sc,stop_sc,step_sc,dwelltime,dwell_ard,self.avg_pts,
																	 expand,offset,sens,filename,folder,self.timestr,self.analogref,self.CM110,self.Ard,self.SR850)
		
		#self.connect(self.get_thread, SIGNAL("make_update3(QString)"), self.make_update3)
		self.connect(self.get_thread, SIGNAL('make_update1(QString,QString,QString,QString,QString)'), self.make_update1)		
	...
	def make_update1(self,all_positions,X_val,endpoint_data,endpoints_times,raw_data):
    
		self.all_wl.extend([ 1e-9*int(all_positions) ])
		self.all_X_volts.extend([ float(X_val) ])
		...
		if len(self.all_wl)>self.schroll_wl:
			...
			self.plot_X_volts[:-1] = self.plot_X_volts[1:]  # shift data in the array one sample left
			self.plot_X_volts[-1] = float(X_val)
			self.plot_wl[:-1] = self.plot_wl[1:]  # shift data in the array one sample left
			self.plot_wl[-1] = 1e-9*float(all_positions)
			self.plot_raw[:-1] = self.plot_raw[1:]  # shift data in the array one sample left
			self.plot_raw[-1] = int(raw_data)
		else:
			self.plot_wl.extend([ 1e-9*int(all_positions) ])
			self.plot_X_volts.extend([ float(X_val)  ])
	...
	def make_update2(self,all_positions,all_data,timelist):
		...
		if len(self.all_wl_tr)>self.schroll_time:
			...
			self.plot_wl_tr[-1] = 1e-9*int(all_positions)
		else:
			self.plot_wl_tr.extend([ 1e-9*int(all_positions) ])
	...
	def set_connect(self):
		...
		try:
			...
			QtGui.QMessageBox.warning(self, 'Message',"No response from CM110 monochromator serial port! Check the port name and check the power line.")
			return None
		...
		try:
			...
			QtGui.QMessageBox.warning(self, 'Message',"No response from Arduino serial port! Check the port name and check the power line.")
			return None
			
		try:
			self.SR850 = SR850.SR850(self.sr850port_str)
			val=self.SR850.set_timeout(2)
			self.SR850.set_to_rs232()
			if not val:
				QtGui.QMessageBox.warning(self, 'Message',"No response from SR850! Check the com line.")
				return None
		except Exception, e:
			QtGui.QMessageBox.warning(self, 'Message',"No response from SR850 lock-in serial port! Check the port name and check com and the power line.")
			return None
	...
	def clear_vars_graphs(self):
		...
		self.pw1.setLabel('left', "Lock-in voltage", units='V')
		...
		self.p1.setLabel('left', "Lock-in voltage", units='V', color='red')
	...
	def set_save(self):
		...
		with open("config_Run_CM110.py", 'w') as thefile:
			...
			thefile.write( ''.join(["sr850port=\"",self.sr850port_str,"\"\n"]) )
	...
	def set_finished(self):
		...
		self.serialCM110.setEnabled(True)
		self.serialArd.setEnabled(True)
		self.serialSR850.setEnabled(True)
