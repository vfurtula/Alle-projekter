...
class Vary_igp:
	...
	def __init__(self):
		...
		self.gw = get_raw.Get_raw()
		self.gmd = get_m_d.Gmd()
	...
	def make_plots(self):
		...
		if self.gw.save_figs:
			string_1 = ''.join([self.gw.filename, '_d_and_m_asf_igp_', self.gw.fit_linear_spline,'_', self.gw.timestr, '.png'])
		...
		if self.gw.save_figs:
			string_2 = ''.join([self.gw.filename, '_RelDisp_and_m_asf_igp_', self.gw.fit_linear_spline,'_', self.gw.timestr, '.png'])
		...
		string_3 = ''.join([self.gw.filename, '_RelDisp_m_d_asf_igp_', self.gw.fit_linear_spline,'_', self.gw.timestr, '.txt'])
		with open(string_3, 'w') as thefile: 
			thefile.write(''.join(['This data is constructed from the config file ', self.gw.last_used_film, '\n']))
