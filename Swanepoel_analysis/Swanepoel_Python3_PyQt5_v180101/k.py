...
class K_class:
	...
	def make_plots(self):
		...
		if self.gw.save_figs:
			string_1 = ''.join([self.gw.filename, '_k_', self.gw.fit_linear_spline,'_', self.gw.timestr, '.png'])
			plt.savefig(string_1)
			pass_plots.extend([string_1])

		string_2 = ''.join([self.gw.filename, '_k_', self.gw.fit_linear_spline,'_', self.gw.timestr, '.txt'])
