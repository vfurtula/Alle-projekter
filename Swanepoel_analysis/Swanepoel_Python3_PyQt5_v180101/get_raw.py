...
class Get_raw:
	...
	def __init__(self):
		...
		try:
			...
			self.gaussian_factors = [float(i) for i in self.config.get(self.last_used_film,'gaussian_factors').strip().split(',')]
			self.gaussian_borders = [float(i) for i in self.config.get(self.last_used_film,'gaussian_borders').strip().split(',')]
			self.ignore_data_pts = int(self.config.get(self.last_used_film,'ignore_data_pts'))
			self.corr_slit = int(self.config.get(self.last_used_film,'corr_slit'))
			self.fit_poly_order = int(self.config.get(self.last_used_film,'fit_poly_order'))
			self.fit_poly_ranges = [float(i) for i in self.config.get(self.last_used_film,'fit_poly_ranges').strip().split(':')[0].split(',')]
			self.fit_poly_ranges_check = self.bool_(self.config.get(self.last_used_film,'fit_poly_ranges').strip().split(':')[1])
			self.filename = self.config.get(self.last_used_film,'filename')
	...
	def make_plots(self):
		...
		string_1 = ''.join([self.filename,'_Tr_subfilmRAW_',self.timestr,'.png'])
		if self.save_figs:
		...
		if len(self.x_Tsubfilm)>0:
			...
			with open(string_2, 'w') as thefile:
				thefile.write(''.join(['This data is constructed from the config file ', self.last_used_film, '\n']))
		...
		if len(self.x_Tsub)>0:
			string_3 = ''.join([self.filename,'_Tr_subRAW_',self.timestr,'.txt'])
