...
class Vary_igp:
	...
	def __init__(self):
		...
		self.ign_pts, self.m_start_min, self.d_dispersion_max, self.d_dispersion_min, self.d_mean, self.dispersion_rel = gmd.get_md_igpo()
	...
	def make_plots(self):
		...
		ax1.plot(self.ign_pts,self.d_mean,'b-',label=''.join(['d_mean (', self.method, ')']))
		...
		if self.save_figs==True:
			string_1 = ''.join([self.folder_str, self.filename_str, '_d_and_m_asf_igp_', self.method,'_', self.timestr, '.png'])
		...
		if self.save_figs==True:
			string_2 = ''.join([self.folder_str, self.filename_str, '_RelDisp_and_m_asf_igp_', self.method,'_', self.timestr, '.png'])
		...
		string_3 = ''.join([self.folder_str, self.filename_str, '_RelDisp_m_d_asf_igp_', self.method,'_', self.timestr, '.txt'])
