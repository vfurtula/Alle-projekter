...
class Run_CM110(QtGui.QWidget):
	...
	def set_run(self):
		...
		if sender.text()=='Plot raw data':
			...
			self.set_save_config()
			import Get_TM_Tm_v0
			self.pp=Get_TM_Tm_v0.Get_TM_Tm()
			self.pp.make_plots()
	...
	def allButtons_torf(self,text):
		...
		self.Step1_Button.setEnabled(text)
		self.Step2_Button.setEnabled(text)
		self.Step3_Button.setEnabled(text)
		self.Step4_Button.setEnabled(text)
		self.Step5_Button.setEnabled(text)
