...
class my_Thread(QThread):
	...
	def run(self):
		...
		with open(data_file, 'w') as thefile:
			...
			thefile.write("Elapsed time [s], B[i][row,column], Voltage [V]\n")
	
		if self.sender=="Run scan":
			
			
			
			for rot_drv,tal in zip(self.drives_active,range(len(self.drives_active))):
				ang_scanlist=numpy.arange(int(10*self.ScanEdit[tal][0]),int(10*self.ScanEdit[tal][1]),int(10*self.ScanEdit[tal][2]))
				positions = self.get_pos(rot_drv,ang_scanlist/10.0)
				
				time_start=time.time()
				for pp in positions:
					self.emit(SIGNAL('clear_graphs_from_thread()'))
					
					min_pos_drv=self.Md.move_Absolute(rot_drv,pp)
					...
					for i,j in zip(self.Rows,self.Colms):
						...
						self.emit(SIGNAL('change_color(PyQt_PyObject)'),'red')
					...
					self.emit(SIGNAL('change_color(PyQt_PyObject)'),'blue')
...
class run_zaber_gui(QtGui.QWidget):
	...
	def initUI(self):
		...
		self.B=[[[ None for i in range(4)] for j in range(4)] for k in range(4)]
		...
		self.conMode = [None for i in range(6)]
		self.disconMode = [None for i in range(6)]
		for i in range(3):
			self.conMode[i] = modeMenu.addAction(''.join(["Connect Drive ", str(i+1)]))
			self.conMode[i].triggered.connect(functools.partial(self.set_connect_drive,i))
			self.conMode[i].setEnabled(False)
			self.disconMode[i] = modeMenu.addAction(''.join(["Disconnect Drive ", str(i+1)]))
		...
		self.calibLoad = calibMenu.addAction("Calib file")
		self.calibLoad.triggered.connect(self.loadCalibDialog)
		self.calibLoad.setShortcut('Ctrl+L')
		self.calibStepperLoad = calibMenu.addAction("Stepperbox calib file")
	...
	def loadCalibDialog(self):
		...
		fname = QtGui.QFileDialog.getOpenFileName(self, 'Open file','/home/vfurtula/Documents/Projects/Scatterometer')
		if fname:
	...
	def scan_func(self):
		...
		for i in range(len(self.drives_active)):
			try:
				passScanEdit[i][0] = int(self.ScanEdit[self.drives_active[i]][0].text())
				passScanEdit[i][1] = int(self.ScanEdit[self.drives_active[i]][1].text())
				passScanEdit[i][2] = int(self.ScanEdit[self.drives_active[i]][2].text())
			except Exception, e:
				QtGui.QMessageBox.warning(self, 'Message',"The scan fields are typed in inconsistently. Possibly real numbers are missing!")
				return None
		...
		self.connect(self.get_my_Thread,SIGNAL("change_color(PyQt_PyObject)"),self.change_color)
	...
	def change_color(self,mystr):
		
		if mystr=="red":
			self.B_lbl[0].setText("B...running")
			self.B_lbl[0].setStyleSheet("color: red")
		elif mystr=="blue":
			self.B_lbl[0].setText("B")
