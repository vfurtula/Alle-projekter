...
class my_Thread(QThread):
	...
	def __init__(self, *argv):
		...
		self.sampl_vals=argv[17]
		self.inpl_vals=argv[18]
		self.outpl_vals=argv[19]
...
class run_zaber_gui(QtGui.QWidget):
	...
	def initUI(self):
		...
		self.setGeometry(30,30,1400,550)
	...
	def loadStepperCalibDialog(self):
		...
		if fname:
			...
			stepper_calib = imp.load_source("",str(fname))
			try:
				...
				QtGui.QMessageBox.warning(self, 'Message',"Could not load stepper calib params! Stepper calib file must have following arrays: sampl,inpl,outpl!")
				return None
	...
	def scan_func(self):
		...
		self.get_my_Thread=my_Thread(sender.text(),self.Esp,self.Md,self.calib_stepper_str,passScanEdit,self.drives_active
															 ,self.B_rotwh,self.PSG_colms,self.PSA_rows,self.Rows,self.Colms,self.channel
															 ,self.avg_pts,self.dwell,filename_str,folder_str,timestr
															 ,self.sampl_vals,self.inpl_vals,self.outpl_vals)
		...
		self.connect(self.get_my_Thread,SIGNAL("update_M()"),self.update_M)
	...
	def update_M(self):
		...
		Bsingle=numpy.zeros((4,4))
		
		try:
			for i in range(4):
				for j in range(4):
					Bsingle[i,j]=int(str(self.B[0,i,j].text()))
					
			Msingle, Mf, Entrophy, DeltaM, DeltaH, Pd = sf.measure_single_Matrix(Bsingle,self.A,self.W)
			for i in range(4):
				for j in range(4):
					self.B[1,i,j].setText(str(round(Msingle[i,j],4)))
		except Exception, e:
				QtGui.QMessageBox.warning(self, 'Message',"Could not calculate Mueller elements! Are all B elements measured? Are calib arrays A and W loaded?")
				return None
