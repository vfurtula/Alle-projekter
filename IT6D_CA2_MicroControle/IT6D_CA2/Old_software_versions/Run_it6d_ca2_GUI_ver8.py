...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def __init__(self):
		...
		self.sample_rate = config_Run_IT6D_CA2.sample_rate
	...
	def initUI(self):
		...
		amp_lbl = QtGui.QLabel("Osc. Vpk-pk[V]", self)
		self.combo7 = QtGui.QComboBox(self)
		mylist7=["0.004","0.1","0.15","0.2","0.25","0.3","0.35","0.4"]
		self.combo7.addItems(mylist7)
		self.combo7.setCurrentIndex(mylist7.index(str(self.lockin_volt)))
		self.combo7.setFixedWidth(100)
		
		freq_lbl = QtGui.QLabel("Osc. freq[Hz]", self)
		...
		g8_1.addWidget(amp_lbl,0,0)
		g8_1.addWidget(self.combo7,1,0)
		g8_1.addWidget(freq_lbl,0,1)
		g8_1.addWidget(self.freqEdit,1,1)
		g8_1.addWidget(rate_lbl,0,2)
		g8_1.addWidget(self.combo8,1,2)
	...
	def allFields(self,trueorfalse):
		...
		self.combo8.setEnabled(trueorfalse)
		
		self.freqEdit.setEnabled(trueorfalse)
	...
	def onActivated8(self, text):
		...
		idx=self.mylist8.index(str(text))
		self.sr810.set_data_sample_rate(idx)
		self.sample_rate=float(text)
		
	def onActivated7(self, text):
		
		self.lockin_volt = float(text)
		self.sr810.set_intrl_volt(self.lockin_volt)
		
		try:
			self.sr810.set_intrl_freq(int(self.freqEdit.text()))
		except Exception,e:
			my_num=int(float(self.freqEdit.text()))
			self.sr810.set_intrl_freq(my_num)
			QtGui.QMessageBox.warning(self, 'Message',''.join(["Oscillator frequency converted to ",str(my_num),"!"]))
			self.freqEdit.setText(str(my_num))
			return None
	...
	def set_run(self):
		...
		if self.op_mode=='xyscan':
			...
			self.onActivated7(self.lockin_volt)
			self.onActivated8(self.sample_rate)
	...
	def set_save(self):
		...
		with open("config_Run_IT6D_CA2.py", 'w') as thefile:
			...
			thefile.write( ''.join(["lockin_freq=",str(self.freqEdit.text()),"\n"]) )
			thefile.write( ''.join(["sample_rate=",str(self.sample_rate),"\n"]) )
