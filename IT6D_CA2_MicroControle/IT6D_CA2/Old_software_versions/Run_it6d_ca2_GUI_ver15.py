...
class Run_IT6D_CA2_Thread(QThread):
	...
	def fit_to_gauss(self,pos,volts):
		...
		# Fit a gaussian
		p1 = poly.polyfit(pos,numpy.log(volts),2)

		fwhm = 2*(2*numpy.log(2)/(-2*p1[2]))**0.5
	  ...
	  ########################################### 
		...
		if self.scan_mode=='xwise':
			...
			for j,tal_outer in zip(y_array,range(len(y_array))):
				...
				p1, fwhm_ = self.fit_to_gauss(numpy.array(pos_x),numpy.array(voltages))
				...
				gauss_volts = numpy.exp(poly.polyval(gauss_pos,p1))
				self.emit(SIGNAL('plot_gaussian(PyQt_PyObject)'),[pos_x,voltages,'y',save_y[-1],gauss_pos,gauss_volts])
			...
			for i,tal_outer in zip(x_array,range(len(x_array))):
				...
				S = self.update_graphs(save_x,save_y,save_Umax,tal_outer)
				p1, fwhm_ = self.fit_to_gauss(numpy.array(pos_y),numpy.array(voltages))
				...
				gauss_volts = numpy.exp(poly.polyval(gauss_pos,p1))
				self.emit(SIGNAL('plot_gaussian(PyQt_PyObject)'),[pos_y,voltages,'x',save_x[-1],gauss_pos,gauss_volts])
	...
	def def_xscan(self):
		...
		gauss_pos = numpy.linspace(min(pos_x),max(pos_x),5*len(pos_x))
		gauss_volts = numpy.exp(poly.polyval(gauss_pos,p1))
		self.emit(SIGNAL('plot_gaussian(PyQt_PyObject)'),[pos_x,voltages,'y',1e-7*int(j_),gauss_pos,gauss_volts])
	...
	def def_yscan(self):
		...
		gauss_pos = numpy.linspace(min(pos_y),max(pos_y),5*len(pos_y))
		gauss_volts = numpy.exp(poly.polyval(gauss_pos,p1))
		self.emit(SIGNAL('plot_gaussian(PyQt_PyObject)'),[pos_y,voltages,'x',1e-6*int(i_),gauss_pos,gauss_volts])
...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def initUI(self):
		...
		self.curve12 = self.p11.plot(pen=None, symbol='s', symbolPen='r', symbolBrush='m', symbolSize=7)
	
		self.p11.enableAutoRange()
		self.p11.setTitle(''.join(["Latest gaussian fit"]))
		...
		p9.setTitle(''.join(["FWHM assuming gaussian"]))
	...
	def plot_gaussian(self,my_object):
		...
		pos_y,voltages,xy_string,save_xy,gauss_pos,gauss_volts = my_object
