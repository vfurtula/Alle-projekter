...
class Run_IT6D_CA2_Thread(QThread):
	...
	def gauss(self,x,p): #p[0]==mean, p[1]==stdev, p[2]==peak height
		...
		return self.fit_he*numpy.exp(-(x-self.mean)**2/(2*p[0]**2))
	
	def fit_to_gauss(self,pos,volts):
		
		# Fit a gaussian
		self.mean = pos[numpy.argmax(volts)]
		self.fit_he = max(volts)
		p0 = [(max(pos)-min(pos))/4] # Initial guess for the gaussian
		errfunc = lambda p,x,y: self.gauss(x,p)-y # Distance to the target function
		p1, success = scop.leastsq(errfunc, p0[:], args=(pos,volts))
		fit_stdev = p1[0]
		
		print fit_stdev, (max(pos)-min(pos))/4
	...
	def fit_to_gauss_(self,pos,volts):
		
		# Fit a gaussian
		p0 = [(max(pos)-min(pos))/2, (max(pos)-min(pos))/4, max(volts)] # Initial guess for the gaussian
		#errfunc = lambda p,x: self.gauss(x,p) # Distance to the target function
		p1 = scop.curve_fit(self.gauss, pos, volts, p0[:])
	...
	def def_xscan(self):
		...
		self.emit(SIGNAL('update_fwhm(PyQt_PyObject,PyQt_PyObject)'),[0],[fwhm_])
		print "FWHM:", fwhm_, "m"
	...
	def set_run(self):
		...
		if self.op_mode=='xyscan':
			...
			if abs(xscan[0]-int(i_)/1000.0)>xy_limiter[0] or abs(xscan[1]-int(i_)/1000.0)>xy_limiter[0]:
				...
				all_pts=len(x_fields)*len(y_fields)*dwelltime+sum(numpy.diff(i_vals))/380.0+sum(numpy.diff(j_vals))/350.0
				...
				if self.smart_scan=="yes":
					if mi==0 and ho==0 and da==0:
						...
						msg=''.join(["Smart scan is activated. The xyscan will take ",str(da)," days, ",str(ho)," hours and ",str(mi)," minutes. Continue?"])
				else:
					if mi==0 and ho==0 and da==0:
						msg=''.join(["The xyscan will take ",str(se)," seconds. Continue?"])
					else:
						msg=''.join(["The xyscan will take ",str(da)," days, ",str(ho)," hours and ",str(mi)," minutes. Continue?"])
			...
			if abs(xscan[0]-int(i_)/1000.0)>xy_limiter[0] or abs(xscan[1]-int(i_)/1000.0)>xy_limiter[0]:
				...
				all_pts=len(x_fields)*dwelltime+sum(numpy.diff(i_vals))/380.0
				mi,se=divmod(int(all_pts),60)
				ho,mi=divmod(mi,60)
				da,ho=divmod(ho,24)
			...
			if abs(yscan[0]-int(j_)/10000.0)>xy_limiter[1] or abs(yscan[1]-int(j_)/10000.0)>xy_limiter[1]:
				...
				all_pts=len(y_fields)*dwelltime+sum(numpy.diff(j_vals))/350.0
				mi,se=divmod(int(all_pts),60)
				ho,mi=divmod(mi,60)
				da,ho=divmod(ho,24)
