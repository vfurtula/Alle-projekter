...
class Run_IT6D_CA2_Thread(QThread):
		...
		if self.scan_mode=='xwise':
			...
			for j,tal_outer in zip(y_array,range(len(y_array))):
				...
				for i in x_array:
					...
					self.emit(SIGNAL('update_graph_lcd(PyQt_PyObject,PyQt_PyObject)'),1e-6*i,1e-7*j)
					time_now=time.time()
					...
					while (time.time()-time_now)<self.dwell_time:
						...
						self.emit(SIGNAL('make_update2(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'),time_elap,1e-6*i,outputcalib)
			...
			for i,tal_outer in zip(x_array,range(len(x_array))):
				...
				for j in y_array:
					...
					while (time.time()-time_now)<self.dwell_time:
						outputcalib=self.get_lockin_data()
						time_elap=time.time()-time_start
						self.emit(SIGNAL('make_update2(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'),time_elap,1e-7*j,outputcalib)
					# get the last voltage value and save to a file
					with open(data_file, 'a') as thefile:
						thefile.write('%s\t' %(1e-6*i))
						thefile.write('%s\t' %(1e-7*j))
	...
	def def_xscan(self):
		...
		for i in self.xscan:
			if self.end_flag==True:
				break
			self.it6d.move_abs('x',i)
			self.emit(SIGNAL('update_graph_lcd(PyQt_PyObject,PyQt_PyObject)'),1e-6*i,1e-7*int(j))
			time_now=time.time()
			# update voltage readouts during dwell time
			while (time.time()-time_now)<self.dwell_time:
				outputcalib=self.get_lockin_data()
			...
			with open(data_file, 'a') as thefile:
				...
				thefile.write('%s\n' %(outputcalib))
			self.emit(SIGNAL('make_update4(PyQt_PyObject,PyQt_PyObject)'),time_elap,outputcalib)
			#self.emit(SIGNAL('make_update3(PyQt_PyObject,PyQt_PyObject)'),time_elap,1e-6*i)
	...
	def def_yscan(self):
		...
		for j in self.yscan:
			if self.end_flag==True:
				break
			self.it6d.move_abs('y',j)
			self.emit(SIGNAL('update_graph_lcd(PyQt_PyObject,PyQt_PyObject)'),1e-6*int(i),1e-7*j)
			time_now=time.time()
			# update voltage readouts during dwell time
			while (time.time()-time_now)<self.dwell_time:
				outputcalib=self.get_lockin_data()
			...
			with open(data_file, 'a') as thefile:
				...
				thefile.write('%s\n' %(outputcalib))
			self.emit(SIGNAL('make_update4(PyQt_PyObject,PyQt_PyObject)'),time_elap,outputcalib)
			#self.emit(SIGNAL('make_update3(PyQt_PyObject,PyQt_PyObject)'),time_elap,1e-7*j)
	...
	def move_rel(self):
		...
		self.emit(SIGNAL('update_lcd(PyQt_PyObject,PyQt_PyObject)'),1e-6*int(i),1e-7*int(j))
...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def initUI(self):
		...
		xy_limiter_lbl = [QtGui.QLabel(i,self) for i in ["X-limiter[mm]","Y-limiter[mm]"]]
		...
		absrel_lbl = QtGui.QLabel("Abs/Rel",self)
	...
	def set_connect(self):
		...
		i,j = self.it6d.get_positions()
		self.update_lcd(1e-6*int(i),1e-7*int(j))
	...
	def set_run(self):
		...
		if self.op_mode=='xyscan':
			...
			if abs(xscan[0]-int(i)/1000.0)>xy_limiter[0] or abs(xscan[1]-int(i)/1000.0)>xy_limiter[0]:
				QtGui.QMessageBox.warning(self, 'Message',''.join([ "All movements in the X direction limited to ", str(xy_limiter[0]) ,' mm' ]))
			elif abs(yscan[0]-int(j)/10000.0)>xy_limiter[1] or abs(yscan[1]-int(j)/10000.0)>xy_limiter[1]:
			...
			i_vals=x_fields
			j_vals=numpy.array([int(j) for jj in range(len(x_fields))])
			...
			j_vals=y_fields
			i_vals=numpy.array([int(i) for ii in range(len(y_fields))])
			
			if abs(yscan[0]-int(j)/10000.0)>xy_limiter[1] or abs(yscan[1]-int(j)/10000.0)>xy_limiter[1]:
				QtGui.QMessageBox.warning(self, 'Message',''.join([ "All movements in the Y direction limited to ", str(xy_limiter[1]) ,' mm' ]))
			elif len(y_fields)<3:
		...
		elif self.op_mode in ['move rel','move abs']:
			
			absrel = [float(self.absrelEdit[ii].text()) for ii in range(2)]
		
			if abs(absrel[0]-int(i)/1000.0)>xy_limiter[0]:
				QtGui.QMessageBox.warning(self, 'Message',''.join([ "All movements in the X direction limited to ", str(xy_limiter[0]) ,' mm' ]))
			elif abs(absrel[1]-int(j)/10000.0)>xy_limiter[1]:
				QtGui.QMessageBox.warning(self, 'Message',''.join([ "All movements in the Y direction limited to ", str(xy_limiter[1]) ,' mm' ]))
			else:
				self.allFields(False)
				self.cancelButton.setEnabled(True)
				...
				self.get_thread=Run_IT6D_CA2_Thread(self.op_mode,self.scan_mode,None,None,absrel_x,absrel_y,
																					None,self.reset_mode,self.timestr,self.it6d,self.sr5210)
