		...
		if self.scan_mode=='xwise':
			...
			for j,tal in zip(y_array,range(len(y_array))):
				...
				for i in x_array:
					...
					# update the position lcd in the GUI
					ii,jj = self.it6d.get_positions()
					self.lcd_actual[0].display(str(int(ii)/1000.0))
					self.lcd_actual[1].display(str(int(jj)/10000.0))
...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def initUI(self):
		...
		lbl7 = QtGui.QLabel("SCAN or MOVE axis:", self)
		...
		if self.op_mode=='xyscan':
			if abs(xscan[0]-int(i)/1000.0)>x_limiter or abs(xscan[1]-int(i)/1000.0)>x_limiter:
				...
				self.get_thread=Run_IT6D_CA2_Thread(self.op_mode,self.scan_mode,folder,filename,xscan,yscan,
																					dwelltime,self.reset_mode,self.timestr,self.it6d,self.sr5210,self.lcd_actual)
				self.all_xy()
			...
			if abs(yscan[0]-int(j)/10000.0)>y_limiter or abs(yscan[1]-int(j)/10000.0)>y_limiter:
				QtGui.QMessageBox.warning(self, 'Message',''.join([ "All movements in the Y direction limited to ", str(y_limiter) ,' mm' ]))
			else:
				self.allFields(False)
				self.cancelButton.setEnabled(True)
