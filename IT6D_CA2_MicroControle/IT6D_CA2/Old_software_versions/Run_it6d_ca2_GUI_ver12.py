...
class Run_IT6D_CA2_Thread(QThread):
	...
	def gauss(self,x,p): #p[0]==mean, p[1]==stdev, p[2]==peak height
		
		return p[2]*numpy.exp(-(x-p[0])**2/(2*p[1]**2))
	
	
	def fit_to_gauss(self,pos,volts):
		
		# Fit a gaussian
		p0 = [(max(pos)-min(pos))/2, (max(pos)-min(pos))/4, max(volts)] # Initial guess for the gaussian
		errfunc = lambda p,x,y: self.gauss(x,p)-y # Distance to the target function
		p1, success = scop.leastsq(errfunc, p0[:], args=(pos,volts))
		fit_mu, fit_stdev, fit_he = p1
	...
	def xyscan(self):
		...
		logdata_Spath_atten=''.join([self.saveinfolder,self.string_filename,"_Spath_spine.txt"])
		...
		data_file_spine_str1 = "Your comment line - do NOT delete this line"
		data_file_spine_str2 = "Axis index, S_path [m], X-pos[m], Y-pos [m], Max voltage [V], FWHM [m]"
	  ...
	  ########################################### 
		...
		if self.scan_mode=='xwise':
			...
			for j,tal_outer in zip(y_array,range(len(y_array))):
				...
				fwhm_ = self.fit_to_gauss(numpy.array(pos_x),numpy.array(voltages))
				
				tals.extend([ tal_outer ])
				fwhm.extend([ fwhm_ ])
				
				with open(data_file_spine, 'w') as thefile:
					thefile.write('%s\n' %data_file_spine_str1)
					thefile.write('%s\n' %data_file_spine_str2)
			...
			for i,tal_outer in zip(x_array,range(len(x_array))):
				...
				S = self.update_graphs(save_x,save_y,save_Umax,tal_outer)
				fwhm_ = self.fit_to_gauss(numpy.array(pos_y),numpy.array(voltages))
...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def initUI(self):
		...
		g3_1.addWidget(self.combo2,1,2)
		...
		self.curve1 = p0.plot(pen='w', symbol='s', symbolPen='w', symbolBrush='b', symbolSize=4)
		self.curve2 = p0.plot(pen='k', symbol='s', symbolPen='k', symbolSize=6)
		
		#self.curve1=pg.ScatterPlotItem(size=5, pen='b', brush='w', pxMode=True)
		#self.pw1.addItem(self.curve1)
		#self.curve2=pg.ScatterPlotItem(size=7, pen='k', pxMode=True)
		#self.pw1.addItem(self.curve2)
	
		self.pw1.enableAutoRange()
		self.pw1.setTitle(''.join(["2-D scan, Jet colormap"]))
		...
		self.curve9=self.p9.plot(pen='m', symbol='d', symbolPen='m',symbolSize=8)
		self.pw6.setTitle(''.join(["FWHM assuming gaussian"]))
		self.pw6.setLabel('left', "FWHM", units='m', color='magenta')
		self.pw6.setLabel('bottom', "Index of axis scanned", color='magenta')
		
		# INITIAL SETTINGS PLOT 7
		p10 = self.pw7.plotItem
		self.curve10=p10.plot(pen='y', symbol='d', symbolPen='y',symbolSize=8)
		self.pw7.setTitle(''.join(["Attenuation slope a"]))
		self.pw7.setLabel('left', "a", units='dB/cm', color='yellow')
	...
	def set_connect(self):
		...
		try:
			...
			if not val:
				QtGui.QMessageBox.warning(self, 'Message',"No response from SR810! Check the serial line to SR810.")
				return None
		except Exception, e:
			QtGui.QMessageBox.warning(self, 'Message',"No response from SR810 port! Check the port name and check the serial and power lines.")
			return None
		...
		try:
			...
			QtGui.QMessageBox.warning(self, 'Message',"No response from IT6D_CA2! Check the port name and check the serial and power lines.")	
			return None
