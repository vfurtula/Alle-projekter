			...
			for j,tal_outer in zip(y_array,range(len(y_array))):
				...
				for i in x_array:
					...
					while (time.time()-time_now)<self.dwell_time:
						outputcalib=self.sr510.return_voltage()
...
class Run_IT6D_CA2(QtGui.QWidget):
	...
	def __init__(self):
		...
		self.sr510port_str=config_Run_IT6D_CA2.sr510port
	...
	def initUI(self):
		...
		self.serSR510 = gpibMenu.addAction("SR510")
		self.serSR510.triggered.connect(self.SR510Dialog)
		...
		self.setWindowTitle("IT6D CA2 Microstepper And SR510 Lock-In Data Acqusition")
		self.show()
	...
	def set_connect(self):
		...
		try:
			...
			#self.sr510.set_timeout(1)
			#self.sr510.return_id()
			#self.sr510.set_timeout(None)
		
		except Exception, e:
			QtGui.QMessageBox.warning(self, 'Message',"No response from SR510! Check the port name and check the com- and power lines.")
		...
		try:
			...
			self.serSR510.setEnabled(False)
	...
	def SR510Dialog(self):
		...
		text, ok = QtGui.QInputDialog.getText(self, 'Serial Port Dialog','Enter SR510 lock-in port number:', text=self.sr510port_str)
		if ok:
		...
		if self.op_mode=='xyscan':
			...
			if abs(xscan[0]-int(i_)/1000.0)>xy_limiter[0] or abs(xscan[1]-int(i_)/1000.0)>xy_limiter[0]:
				...
				self.get_thread=Run_IT6D_CA2_Thread(self.op_mode,self.scan_mode,folder,filename,x_fields,y_fields,
																					dwelltime,self.reset_mode,self.timestr,self.it6d,self.sr510)
			...
			if abs(abss[0]-int(i_)/1000.0)>xy_limiter[0]:
				...
				self.get_thread=Run_IT6D_CA2_Thread(self.op_mode,self.scan_mode,None,None,absrel_x,absrel_y,
																					None,self.reset_mode,self.timestr,self.it6d,self.sr510)
				self.connect(self.get_thread, SIGNAL('update_lcd(PyQt_PyObject,PyQt_PyObject)'), self.update_lcd)
				self.connect(self.get_thread, SIGNAL('finished()'), self.set_finished)
				self.get_thread.start()
