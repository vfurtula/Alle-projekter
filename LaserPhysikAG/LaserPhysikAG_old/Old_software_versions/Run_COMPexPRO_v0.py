...
class Run_COMPexPRO_Thread(QThread):
	...
	def warmup(self):
		...
		for tal in range(480)[::-1]:
			if tal==0:
				self.warmup_lcd.setStyleSheet("color: green")
			self.warmup_lcd.display(str(tal))
			time.sleep(1)
	...
	def start_source(self):
		...
		print self.COMPexPRO.set_opmode('ON')
		...
		print self.COMPexPRO.set_opmode('OFF')
...
class Run_COMPexPRO(QtGui.QWidget):
	...
	def initUI(self):
		...
		warmup_lbl = QtGui.QLabel("Warm-up", self)
		...
		gas_wl_lbl = QtGui.QLabel("Wavelength:",self)
		
		self.gas_mix = QtGui.QLabel("",self)
		self.gas_mix.setStyleSheet("color: magenta")
		gas_mix_lbl = QtGui.QLabel("Gas mixture:",self)
		
		pulse_lcd_lbl = QtGui.QLabel("Counter (10^3)",self)
		self.pulse_lcd = QtGui.QLCDNumber(self)
		self.pulse_lcd.setStyleSheet("color: blue")
		self.pulse_lcd.setFixedHeight(50)
		self.pulse_lcd.setSegmentStyle(QtGui.QLCDNumber.Flat)
		self.pulse_lcd.setNumDigits(4)
		self.pulse_lcd.display('----')
		
		pulse_lcd2_lbl = QtGui.QLabel("Total counter (10^3)",self)
		self.pulse_lcd2 = QtGui.QLCDNumber(self)
		self.pulse_lcd2.setStyleSheet("color: blue")
		#self.pulse_lcd2.setFixedHeight(45)
		self.pulse_lcd2.setSegmentStyle(QtGui.QLCDNumber.Flat)
		self.pulse_lcd2.setNumDigits(6)
		self.pulse_lcd2.display('------')
		
		warmup_lcd_lbl = QtGui.QLabel("Warm-up [s]",self)
		self.warmup_lcd = QtGui.QLCDNumber(self)
		#self.warmup_lcd.setFixedHeight(45)
		self.warmup_lcd.setSegmentStyle(QtGui.QLCDNumber.Flat)
		self.warmup_lcd.setNumDigits(3)
		if self.warmup_str=="ON":
			self.warmup_lcd.setStyleSheet("color: red")
			self.warmup_lcd.display('480')
		else:
			self.warmup_lcd.setStyleSheet("color: green")
			self.warmup_lcd.display('0')
		...
		g1_5.addWidget(gas_menu_lbl,0,0)
		g1_5.addWidget(self.gas_menu,0,1)
		g1_5.addWidget(gas_wl_lbl,0,2)
		g1_5.addWidget(self.gas_wl,0,3)
		g1_5.addWidget(gas_mix_lbl,0,4)
		g1_5.addWidget(self.gas_mix,0,5)
		
		g1_3 = QtGui.QGridLayout()
		g1_3.addWidget(pulse_lcd_lbl,0,0)
		g1_3.addWidget(self.pulse_lcd,1,0)
		g1_3.addWidget(pulse_lcd2_lbl,0,1)
		g1_3.addWidget(self.pulse_lcd2,1,1)
		g1_3.addWidget(warmup_lcd_lbl,0,2)
		g1_3.addWidget(self.warmup_lcd,1,2)
		
		g1_4 = QtGui.QGridLayout()
		g1_4.addWidget(self.resetButton,0,0)
		g1_4.addWidget(pulse_time_lbl,0,1)
		g1_4.addWidget(self.pulsetimeEdit,0,2)
	...
	def update_pulse_lcd(self,i):
		...
		self.pulse_lcd.display(i)
	
	def update_pulse_lcd2(self,i):
		# i and j are always in meters
		self.pulse_lcd2.display(i)
		
	def set_reset(self):
		
		self.pulse_lcd.display(self.COMPexPRO.set_counter_reset())
	...
	def set_connect(self):
		...
		try:
			...
			self.pulse_lcd.display(self.COMPexPRO.get_counter())
			self.pulse_lcd2.display(self.COMPexPRO.get_totalcounter())
		except Exception, e:
			QtGui.QMessageBox.warning(self, 'Message',"No response from the COMPexPRO photon source! Check the port name and check serial- and power lines.")	
			return None
	...
	def set_disconnect(self):
		...
		print "COMPexPRO trigger set to "+self.COMPexPRO.set_trigger('EXT')
	...
	def set_start(self):
		...
		self.get_thread=Run_COMPexPRO_Thread(self.rate,self.hv,self.timeout_str,self.warmup_str,self.warmup_lcd,self.trigger_str, self.gasmode_str, self.pulse_time,self.data_file,self.timetrace_str,self.COMPexPRO,self.stopButton)
