		...
		for volt in self.Vsweep:
			...
			while time.time()-time_start_<self.pulse_time:
				...
				if self.inst_list.get('COMPexPRO'):
					...
					hv = self.inst_list.get('COMPexPRO').get_hv(volt)
					rate = self.inst_list.get('COMPexPRO').get_reprate(self.rate)
				...
				if self.inst_list.get('COMPexPRO') and self.inst_list.get('PM100USB'):
					self.signals.make_update1.emit(time_elap,1e-3*egy,pm_power,hv,rate)
					...
					with open(self.data_file, "a") as thefile:
						thefile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(time_elap,totalcounter,rate,hv,egy,lt_press,lt_temp,buffer_press,pm_power))
						
				elif self.inst_list.get('COMPexPRO') and not self.inst_list.get('PM100USB'):
					self.signals.make_update1.emit(time_elap,1e-3*egy,-1,hv,rate)
					self.signals.make_update2.emit(time_elap,1e-3*lt_press,lt_temp)
					self.signals.make_update3.emit(time_elap,1e-3*buffer_press)
					self.signals.make_update4.emit(time_elap,1e-3*f2_press,f2_temp)
					self.signals.make_update5.emit(time_elap,pulse_diff)
					...
					with open(self.data_file, "a") as thefile:
						thefile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(time_elap,totalcounter,rate,hv,egy,lt_press,lt_temp,buffer_press))
						
				elif self.inst_list.get('PM100USB') and not self.inst_list.get('COMPexPRO'):
					self.signals.make_update1.emit(time_elap,-1,pm_power,hv,rate)
...
class Run_COMPexPRO(QMainWindow):
	...
	def set_run(self):
		...
		########################################################
		print(self.VsweepEdit.text().split(','))
		
		if len(self.VsweepEdit.text().split(','))==1 or len(self.VsweepEdit.text().split(','))==3:
			pass
		else:
		...
		try:
			...
			if len(Vsss)==3:
				hv = numpy.arange(10*Vsss[0],10*(Vsss[1]+Vsss[2]),10*Vsss[2])/10
				if hv.size==0:
	...
	def make_update1(self,time_elap,energy,power,hv,rate):
	...
	def load_(self):
		...
		try:
			...
			self.volt = int(self.config.get(self.last_used_scan,"volt"))
