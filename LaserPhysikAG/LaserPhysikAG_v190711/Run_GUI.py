...
import os, sys, re, time, numpy, yagmail, shutil, configparser, traceback
...
class WorkerSignals(QObject):
	...
	make_update2 = pyqtSignal(object,object)
...
class Run_COMPexPRO_Thread(QRunnable):
	...
	def abort(self):
		...
		if self.stopButton.text()=="STOP source":
			if self.inst_list.get('PM100USB'):
				self.inst_list.get('PM100USB').abort()
			if self.inst_list.get('COMPexPRO'):
				str_on = self.inst_list.get('COMPexPRO').set_opmode("OFF")
				print("Stopped, ",str_on)
			self.stopButton.setEnabled(True)
		elif self.stopButton.text()=="SKIP warm-up":
			pass
	...
	def start_source(self):
		...
		if self.inst_list.get('PM100USB'):
			self.inst_list.get('PM100USB').setTimeoutValue(11000)
			...
			self.inst_list.get('PM100USB').setEnergyRange(0.01)
		...
		for rate in self.Rsweep:
			for volt in self.Vsweep:
				...
				if self.inst_list.get('COMPexPRO'):
					...
					self.signals.make_update2.emit(volt,rate)
					if str_on[:3]=="OFF":
				...
				while time.time()-time_start_<self.pulse_time:
					...
					self.signals.make_update1.emit(time_elap,1e-3*egy,pm_energy)
					...
					with open(self.data_file, "a") as thefile:
						thefile.write("%s\t%s\t%s\t%s\t%s\t%s\n" %(time_elap,totalcounter,rate,volt,egy,pm_energy))
...
class Run_COMPexPRO(QMainWindow):
	...
	def initUI(self):
		...
		self.VsweepEdit.setFixedWidth(150)
		
		self.Rsweep_lbl = QLabel("".join(["Rep. rate (",str(self.rate),") [Hz]"]), self)
		self.RsweepEdit = QLineEdit(",".join([i for i in self.Rsweep]),self)
		self.RsweepEdit.setFixedWidth(140)
		
		pulse_time_lbl = QLabel("Pulsing time [s]",self)
		self.pulsetimeEdit = QLineEdit(str(self.pulse_time),self)
		self.pulsetimeEdit.setFixedWidth(120)
		...
		g1_2 = QGridLayout()
		g1_2.addWidget(self.Rsweep_lbl,0,0)
		g1_2.addWidget(self.RsweepEdit,1,0)
		g1_2.addWidget(self.Vsweep_lbl,0,1)
		g1_2.addWidget(self.VsweepEdit,1,1)
		g1_2.addWidget(pulse_time_lbl,0,2)
		g1_2.addWidget(self.pulsetimeEdit,1,2)
		...
		g1_3.addWidget(self.resetButton,3,0)
		g1_3.addWidget(warmup_lbl2,3,2)
		g1_3.addWidget(self.warmup,3,3)
		...
		self.vb0 = pg.ViewBox()
		self.curve1_1=pg.PlotCurveItem(pen="r")
		self.vb0.addItem(self.curve1_1)
		
		ax0 = pg.AxisItem('right',pen="r")
		ax0.setLabel("Energy", units="J", color="white")
		self.pi1.layout.addItem(ax0,2,3)
		self.pi1.scene().addItem(self.vb0)
		ax0.linkToView(self.vb0)
		self.vb0.setXLink(self.pi1)
		
		# create plot and add it to the figure
		self.vb1 = pg.ViewBox()
		self.curve1_2=pg.PlotCurveItem(pen="y")
		...
		self.pw1.setTitle("".join(["Current rate ",str(self.rate)," Hz, Voltage ",str(self.volt)," kV"]))
		self.pw1.setLabel("right","Energy", units="J", color="white")
		self.pw1.setLabel("left", "Voltage", units="V", color="white")
	...
	def initUI_(self):
		...
		self.Rsweep_lbl.setText("".join(["Rep. rate (",str(self.rate),") [Hz]"]))
	...
	def set_run(self):
		...
		self.worker = Run_COMPexPRO_Thread(rate,hv,self.timeout_str,self.warmup_bool,self.warmup,self.trigger_str, self.gasmode_str, self.pulse_time,self.data_file,self.timetrace_str,self.inst_list,self.stopButton)
	...
	def make_update1(self,time_elap,energy,power):
		...
		def updateViews():
			...
			self.vb0.linkedViewChanged(self.pi1.vb, self.vb0.XAxis)
	...
	def save_(self):
		...
		self.config.set(self.last_used_scan,"Rsweep", str(self.RsweepEdit.text()) )
