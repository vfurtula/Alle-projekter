	...
	def pm100usb(self):
		...
		if self.cb_testmode.isChecked() and self.cb_pm100usb.isChecked():
			...
			try:
				...
				self.pm100usb_status.setStyleSheet("color: green")
				self.inst_list.update({"PM100USB":self.PM100USB})
				self.inst_list.update({"PM100USB_tm":False})
				...
				if reply == QMessageBox.Yes:
					...
					self.inst_list.update({"PM100USB":self.PM100USB})
					self.inst_list.update({"PM100USB_tm":True})
					self.cb_pm100usb.setChecked(True)
	...
	def compexpro(self):
		...
		if not self.cb_testmode.isChecked() and self.cb_compexPro.isChecked():
			try:
				...
				self.compexPro_status.setStyleSheet("color: green")
				self.inst_list.update({"COMPexPRO":self.COMPexPRO})
				self.inst_list.update({"COMPexPRO_tm":False})
			except Exception as e:
				reply = QMessageBox.critical(self, 'COMPexPRO testmode', ''.join(["COMPexPRO could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nProceed into the testmode?"]), QMessageBox.Yes | QMessageBox.No)
				if reply == QMessageBox.Yes:
					self.COMPexPRO = COMPexPRO.COMPexPRO(self.compexproport_str, True)
					self.compexPro_status.setText("Testmode: CONNECTED")
					self.compexPro_status.setStyleSheet("color: magenta")
					self.inst_list.update({"COMPexPRO":self.COMPexPRO})
					self.inst_list.update({"COMPexPRO_tm":True})
					self.cb_compexPro.setChecked(True)
