...
class Ref_typeB:
  def __init__(self,my_string):
    ...
    for line in f2:
      line = line.strip()
      columns = line.split()
      self.temp.extend([ 22+float(columns[0]) ])
      self.volt.extend([ 1e3*float(columns[1]) ])
    f2.close()
...
if __name__ == "__main__":
  ...
  plt.xlabel("Voltage [uV]", fontsize=20)
  ...
  #brake_min=1
  #brake_max=892
  #common_volts = volt_bit_space[brake_min:brake_max+1]
  #common_bits = bit_space[brake_min:brake_max+1]
  common_volts = volt_bit_space
  common_bits = bit_space
  
  # 1 bit = 1.075 mV assuming 1024 bits for 1.1V (Ref Voltage for MEGA2560)
  f = interpolate.splrep(ampl_volt, temp, k=1, s=0)
  temp_space = interpolate.splev(common_volts, f, der=0)
  # Check your results
  print temp_space[0]
  print temp_space[-1]
  ...
  plt.plot(common_volts[:-1], np.diff(temp_space),'ko', label="derivative")
  plt.ylabel("$\Delta$ Temp/$\Delta$ bit [C/bit]", fontsize=20)
  plt.xlabel("Voltage [uV]", fontsize=20)
  plt.tick_params(axis="both", labelsize=20)
  #plt.yticks( np.linspace(0,1,11) )
  #plt.xticks( np.linspace(0,11000,12) )
  ...
  plt.ylabel("$\Delta$Temp/1 bit [C/bit]", fontsize=20)
  ...
  plt.plot(common_bits, temp_space,'ko', label="Temp[C]")
  plt.ylabel("Temp [C]", fontsize=20)
